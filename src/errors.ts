...
const UNDEFINED_PARSER_CONTEXT = new Error(
  "Cannot operate on undefined parser context"
);
const INVALID_HEADER = new Error("Header values can only be strings!");
const ILLEGAL_ADD = new Error("Can only add values of the same type!");
export {
  VAR_NOT_FOUND,
  ILLEGAL_EXTRACTION,
  ILLEGAL_SET,
  UNDEFINED_PARSER_CONTEXT,
  INVALID_HEADER,
  ILLEGAL_ADD,
};
